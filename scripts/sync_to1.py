...
def build(raw):
    ...
    assert rows[4][11].startswith('Lũy kế giá trị thực hiện') and rows[4][12].startswith('Giá trị khối lượng thực hiện')
    assert 'triệu đồng' in rows[4][11] and 'triệu đồng' in rows[4][12]
    assert rows[4][13].startswith('Tóm tắt khó khăn')
    projects=[]; totals_row=None
    for ix,r in enumerate(rows[6:],7):
        assert len(r)==14, 'Unexpected column count'
        r=list(r)
        ...
        p=dict(id=100000+stt,stt=stt,sheet='Báo cáo',row=ix,group=group,name=r[1],investment=inv,plan=plan,paid=paid,left=left,base=plan,rate=rate,owner='Ban Quản lý các Khu kinh tế và khu công nghiệp tỉnh',difficulty=r[13],progress=r[10],note=None,raw=r,sourceHeaders=headers,sourceFile='Theo dõi giải ngân Tổ công tác số 1',sourceUrl=URL,leaderOrigin='Tổ 1 · PCT Thường trực Hồ Thị Nguyên Thảo (phân công theo yêu cầu)',rateOrigin='Tỷ lệ ghi trong nguồn',baseOrigin='KH vốn 2026, triệu đồng quy đổi sang tỷ đồng (chia 1.000)',quality=[],status='blank' if not r[13].strip() else ('no' if r[13].strip().lower() in ('không','không có') else 'yes'),monthlyPaid=monthly,monthlyPlan=monthlyplan,monthlyRate=r[9],appendices=[],sources=[],pdfSources=[])
        p.update({k:None for k in ['carry','current','paidCarry','paidCurrent','leftCarry','leftCurrent']})
        p['performedCumulative']=None if number(r[11]) is None else float(number(r[11])/1000)
        p['performedDaily']=None if number(r[12]) is None else float(number(r[12])/1000)
        projects.append(p)
    assert projects and totals_row is not None
    for col,key in [(2,'investment'),(3,'plan'),(4,'paid'),(5,'left'),(7,'monthlyPaid'),(8,'monthlyPlan'),(11,'performedCumulative'),(12,'performedDaily')]:
    ...
    data['to1Source']=dict(url=URL,date=rows[0][13],sha256=hashlib.sha256(raw).hexdigest(),headers=headers,totals=totals_row)
    def audit(pp):
